...
class StorageFragment(
    ...
    // "Cache" here is each app's cacheBytes as reported by StorageStatsManager (the same number
    // shown in system Settings > Apps > [app] > Storage > Clear cache). This is deliberately NOT
    // a walk of Android/data/<pkg>/cache on disk: since Android 11, that directory is blocked at
    // the filesystem level for every app except the one that owns it, even with All Files Access
    // (MANAGE_EXTERNAL_STORAGE) granted — Google's own docs list Android/data as an explicit
    // exclusion. StorageStatsManager is the only route left that still works without root, via
    // the same "usage access" permission already used for the Apps total (checkAppOpsService()).
    // Trade-off: this API only reports a per-app byte total, not individual file paths, so the
    // "count" here is the number of apps that currently have cache data — not a file count like
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            packages.chunked(chunkSize).forEach { chunk ->
                val results = chunk.map { appInfo ->
                    async {
                        try {
                            storageStatsManager.queryStatsForPackage(uuid, appInfo.packageName, userHandle).cacheBytes
                        } catch (e: Exception) {
                            0L
                        }
                    }
            ...
            context.config.saveCachedCategorySize(primaryVolumeName, "cache", totalCache)
            context.config.saveCachedCategoryCount(primaryVolumeName, "cache", appsWithCache)
            post {
    ...
    private fun getSizesByMimeType(volumeName: String): Pair<HashMap<String, Long>, HashMap<String, Int>> {
        ...
        try {
            context.queryCursor(uri, projection) { cursor ->
                try {
                    // Match MimeTypesActivity's browsable list, which skips 0-byte rows entirely
                    // for every category — otherwise a category's count/size here can include
                    // files that silently vanish (never render) when the user actually taps in.
