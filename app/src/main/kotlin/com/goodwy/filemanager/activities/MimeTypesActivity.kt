...
class MimeTypesActivity : SimpleActivity(), ItemOperationsListener {
    ...
    private fun buildMimeTypeSelection(): Pair<String, Array<String>> {
        ...
        // Source files (.kt/.py/.md/...) usually come back with no MIME type at all, or with the
        // generic application/octet-stream fallback, so a mimetype-only rule can never catch them.
        // Match on filename extension instead and treat those as documents.
        fun documentExtensionCondition(): Pair<String, List<String>> {
            val nameCol = MediaStore.Files.FileColumns.DISPLAY_NAME
            val condArgs = mutableListOf<String>()
            val extConditions = documentExtensions.joinToString(" OR ") { "$nameCol LIKE ?" }
            documentExtensions.forEach { condArgs.add("%.$it") }
            return Pair("($extConditions)", condArgs)
        }
        ...
        //
        // The document-extension rule has to be applied out here rather than inside the mimetype
        // branches above: source files are exactly the ones that come back with a NULL mimetype, so
        // folding the rule into `selection` would put it behind the "$col IS NOT NULL" guard and it
        // would never match the files it exists to catch.
        val (docExtCond, docExtArgs) = documentExtensionCondition()

        val fullSelection = when (currentMimeType) {
            DOCUMENTS -> {
                val mimeMatch = if (selection.isNotEmpty()) "($col IS NOT NULL AND ($selection))" else "0"
                args.addAll(docExtArgs)
                "$mimeMatch OR $docExtCond"
            }

            OTHERS -> {
                val base = if (selection.isNotEmpty()) "($col IS NULL OR ($col IS NOT NULL AND ($selection)))" else "1"
                args.addAll(docExtArgs)
                "$base AND NOT $docExtCond"
            }

            else -> "$col IS NOT NULL" + if (selection.isNotEmpty()) " AND (${selection})" else ""
        }
